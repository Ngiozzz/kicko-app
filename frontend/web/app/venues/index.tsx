...
// Public (no login) venue directory — see app/player/explore/index.tsx for
// the logged-in equivalent this was adapted from. Same filter/grid UI,
// fed from /api/public/venues* instead, so anyone (a visitor, or Google)
// can browse without an account. Booking still requires signing in, from
// the detail page ([id].tsx).
...
export default function PublicVenues() {
  ...
  const load = useCallback(async () => {
    try {
      const { venues } = await publicVenuesApi.list();
      setVenues(venues);
    } catch (err) {
    ...
    <ScrollView style={styles.root}>
      ...
      <View style={styles.page}>
        ...
        <View style={styles.resultsBar}>
          <Text style={styles.resultsText}>{venues === null ? '—' : `Showing ${sorted.length} of ${venues.length}`}</Text>
