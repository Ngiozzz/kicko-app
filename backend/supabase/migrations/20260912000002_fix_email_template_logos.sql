-- The logo mark in every template was inline <svg> — Gmail strips
-- <svg> out of HTML emails entirely, so it rendered blank there. Swaps
-- it for a real hosted <img> (the same PNG the old wrapper() banner
-- already used successfully), preserving each template's audience
-- badge ("for Owners" / "for Organizers" / "Admin") next to it.
-- Re-seeds all 17 rows from the updated docs/email templates/*.html.
